...
for valor in range(x):
    inserir_nmr = int(input())
    
    if inserir_nmr == 0:
        classificacao = "NULL"
    elif (inserir_nmr % 2) == 0 and inserir_nmr > 0:
        classificacao = "EVEN POSITIVE"
    elif (inserir_nmr % 2) == 0 and inserir_nmr < 0:
        classificacao = "EVEN NEGATIVE"
    elif (inserir_nmr % 2) != 0 and inserir_nmr > 0:
        classificacao = "ODD POSITIVE"
    elif (inserir_nmr % 2) != 0 and inserir_nmr < 0:
        classificacao = "ODD NEGATIVE"
    
    arm_valores.append(classificacao)


for classificacoes in arm_valores:
